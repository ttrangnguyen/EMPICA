...
if __name__ == "__main__":
    ...
    for field_name in columns:
        count = 0
        file_name = "deepseek-coder-6.7_method_name_"+ field_name +"_java.csv"
        for idx, row in df.iterrows():
            print("analyzing:", idx)
            data = {}
            try:
                source_content = df.at[idx, "function"]
                ...
                chat = [
                    {"role": "user", "content": """Generate the function name for the given function: \n""" + source_content},
                ]
                ...
                response = tmp
                data[idx] = {"task_id": df.at[idx, "task_id"], "original_method": method_name, "response": response}
